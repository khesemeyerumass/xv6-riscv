...
// Macros used to specify which allocator should be compiled
//#define DEFAULTALLOC
#define BUDDYALLOC
    ...
    void binit() {
        ...
        root->size = 2147483648;
        root->isSplit = 0;
        root->isUsed = 0;
        root->mem = (void*) sbrk(2147483648);
    }
    ...
    // Function used when there isn't enough free memory for an allocation request. It will
    // create another root node (start of another binary tree of free memory) the root will be
    // returned by the function.
    static Node* addMoreFreeMemory() {
        Node* newRoot = (Node*) sbrk(4096 * sizeof(Node));
        newRoot->size = 2147483648;
        newRoot->isSplit = 0;
        newRoot->isUsed = 0;
        newRoot->mem = (void*) sbrk(2147483648);
        
        // Add newRoot to the root pointer of the previous root node (essentially forming
        // a linked list of root nodes staring with the original root node)
        Node* curr = root;
        while (curr->nextRoot != 0) {
            curr = curr->nextRoot;
        }
        curr->nextRoot = newRoot;
        return newRoot;
    }
#endif
...
malloc(uint nbytes)
{
      ...
      // Search each tree for an available memory block that is of valid size
      Node* curr = 0;
      Node* currRoot = root;
      while (currRoot != 0 && curr == 0) {
          curr = findAvailableMem(currRoot, nbytes);
          if (curr == 0) {
      ...
      if (curr == 0) {
          Node* newRoot = addMoreFreeMemory();
          curr = findAvailableMem(newRoot, nbytes);
      }
